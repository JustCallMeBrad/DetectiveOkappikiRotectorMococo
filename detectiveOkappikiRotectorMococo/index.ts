...
export default definePlugin({
    ...
    patches: [
        {
            ...
            replacement: [
                {
                    match: /(let\{user:(\i),currentUser:\i,displayProfile:\i,guildId:\i,channelId:\i,items:(\i),initialSection:\i,onClose:\i\}=\i,)/,
                    replace: "$1_dormInit=$self.pushSection($3,$2),"
                },
                {
    ...
    pushSection(sections: any[], user: any) {
        if (!sections.some((s: any) => s.section === "USER_FLAGS")) {
